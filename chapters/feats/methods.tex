...
% We use the superpixel segmentation algorithm SLIC proposed in \cite{achanta12}.
...
% However, as we compare between different methods, seeding the random number generators would not allow a fair comparison.
...
% Its settings are described in chapter \ref{random_forest}.
% For evaluation, the Random Forest is not seeded.
...
% We experimented with the patch size used for the \textit{ScSP} and \textit{VGG-16} methods.
