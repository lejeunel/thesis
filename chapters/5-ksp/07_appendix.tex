...
\DontPrintSemicolon
\Input{$G$: Directed Acyclic Graph constructed as in Sec.~\ref{sec:solving}}
\Output{$P$: Set of K-shortest paths}
$p_0 \gets$ \texttt{bellman\_ford\_shortest\_paths}$(G)$\;
$P_0 \gets\{p_0\} $\;
\For{$l\gets0$ \KwTo $l_{max}$ }{
 \If{$l \neq 0$}{
   \If{$\text{cost}(P_l) \geq \text{cost}(P_{l-1})$}{
     \Return $P_{l-1}$
     }
 }
   $G_r \gets$ \texttt{reverse}$(G,P_l)$\tcp*{Reverse edges directions and algebraic signs}
   $G_r^+ \gets$ \texttt{edge\_costs\_transform}$(G_r)$\tcp*{As in Eq.~\eqref{eq:cost_transform}}
   $\tilde{p}_l \gets$ \texttt{dijkstra\_shortest\_paths}$(G_r^+)$\tcp*{Returns interlacing path}
   $P_{l+1} \gets$ \texttt{augment}$(P_l, \tilde{p}_l)$\tcp*{As on Fig.~\ref{fig:augment}}
